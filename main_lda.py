...
b = sklearnLDA_NMF.remote(model, df_reviews, config1["learning_decay"], config1["alpha"], config1["eta"],\
     config1["epochs"], config1["learning_method"], config1["learning_offset"], config1["batch_size"],\
         config1["num_topics"], config1["num_words"], config1["random_state"], (config1["ngram_min"], config1["ngram_max"]),\
         config1["min_df"], config1["max_df"], config1["max_features"])
[lda, vectorizer_cvLDA, X_cv, topics, df_doc_topic, significant_topic] = ray.get(b.topic_gen.remote())
df_topic_table = ray.get(b.table_formatting.remote(df_doc_topic))
df_topic_table.to_excel("Results/df_topic_table_lda_%d.xlsx"%config1["title_count"])
ray.get(b.get_model_topics.remote(lda_topics, config1["n_top_words"])).to_excel("Topics/topics_lda_%d.xlsx"%config1["title_count"])
ray.shutdown()
...
##Saving the model
joblib.dump(vectorizer_cvLDA, os.path.join(config["model_directory"], "vectorizer_cvlda.pkl"))
joblib.dump(lda, os.path.join(config["model_directory"], config["model_name_lda"]))